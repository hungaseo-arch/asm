...
import type { NewPurchaseOrderInput, PurchaseOrder, SortKey } from '@/types/purchase-po'
...
export const usePurchasePoStore = defineStore('purchase-po', () => {
  // ── 상태 (State / Status) ────────────────────────────────────────────────
  const orders = ref<PurchaseOrder[]>([...seedPurchaseOrders])
  ...
  const page = ref(1)
  const sortKey = ref<SortKey>('poDate')
  ...
  function addOrder(input: NewPurchaseOrderInput): PurchaseOrder {
