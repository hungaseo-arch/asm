...
--
-- ── 왜 v3 인가 ─────────────────────────────────────────────────────────────
-- v1(TEMP TABLE + 함수 + 트랜잭션), v2(DO 블록) 모두 Neon SQL Editor 에서 "실행 완료" 뒤에도
-- 데이터가 그대로였습니다(Data API 로 실측). 편집기가 $$ 블록을 문장 단위로 쪼개는 것으로 보여,
-- 이번에는 **달러 인용 없이 순수 SQL 두 문장**(백업 INSERT · UPDATE)만 둡니다.
-- 문장마다 따로 실행해도 됩니다. 두 번 실행해도 결과는 같습니다.
--
-- ── 왜 바꾸는가 ────────────────────────────────────────────────────────────
