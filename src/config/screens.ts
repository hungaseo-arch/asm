 ...
 *
 * 구성과 순서는 개선의견서(ASM 실서버 테스트 결과)의 메뉴 목차를 따르고,
 * 경로(path)는 작업지시서 §5 에 따라 실서버 경로명을 그대로 사용합니다.
...
export const screenRoutes: ScreenRoute[] = [
  ...
  { slug: 'stock-movement', path: '/stock-movement', title: 'Stock Movement',
    load: () => import('@/data/screens/stock-movement').then((m) => m.stockMovementScreen) },
  { slug: 'stock-adjustment', path: '/inventory-adjust', title: 'Stock Adjustment',
    load: () => import('@/data/screens/stock-adjustment').then((m) => m.stockAdjustmentScreen) },
  { slug: 'stock-transfer', path: '/stock-transfer', title: 'Stock Transfer',
    load: () => import('@/data/screens/stock-transfer').then((m) => m.stockTransferScreen) },
  { slug: 'stock-opname', path: '/stock-opname', title: 'Stock Opname',
    load: () => import('@/data/screens/stock-opname').then((m) => m.stockOpnameScreen) },
  { slug: 'brand-pattern', path: '/brand-pattern', title: 'Brand / Pattern',
