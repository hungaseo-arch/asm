...
import { AGED_STOCK_DAYS, valueOf, type InventoryItem, type InventorySortKey } from '@/types/inventory'

export const useInventoryStore = defineStore('inventory', () => {
  // ── 상태 (State / Status) ────────────────────────────────────────────────
  const items = ref<InventoryItem[]>([...seedInventory])
  ...
  const page = ref(1)
  const sortKey = ref<InventorySortKey>('qty')
  const sortAsc = ref(false)

  // ── 선택지 (Filter options / Pilihan filter) ────────────────────────────
  const distinct = (key: 'wh' | 'cat' | 'brand') =>
