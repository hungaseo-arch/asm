...
| 3 | `/csr/13` 검증 추가 REJECTED → 헤더 자동 갱신 | **확인(서종환, 2026-09-11 00:45 UTC)** — 이력 269(REJECTED, 09-11) 등록 → 헤더 최종검증일 09-10 → 09-11 자동 갱신(로그 74). ※ 첫 시도는 헤더 「편집」으로 결과만 바꿔 날짜가 안 변한 것(정상 동작) |
...
- **주의**: H-2 직후(15:52 UTC) 30 의 현업검증이 편집 저장으로 `ACCEPTED → PENDING` 으로 바뀜(로그 70, 이력 추가 없음). 편집 폼은
  현재 값을 그대로 띄우므로(04 번 실측 ACCEPTED) 셀렉트에서 고른 값이 저장된 것. 의도하지 않은 값으로 확인되어 2026-09-11 ACCEPTED 로 복구(로그 73).

## 배포 후 추가 수정 (2026-09-11)

- **로그아웃 불가** — 프로덕션 빌드가 `client.signOut?.()` 을 `signOut.call(client)` 로 바꾸고, better-auth 프록시가 `.call` 을 API 경로로
  읽어 클라이언트를 첫 인자로 넘김 → `GET /fetch-options/method/to-upper-case` 404, 실제 sign-out 미호출. 개발 서버에서는 변환이 없어
  정상. 프록시 메서드의 옵셔널 호출을 전부 직접 호출로 바꾸고(`token()` · `admin.removeUser()` 포함) `/sign-out` 직접 호출을 보강.
  배포본 실측: sign-out 200, 세션 쿠키 삭제, 헤더 이름 사라짐, 새로고침 후 로그인 폼.
- **캡쳐 썸네일** — 로그인된 Google 계정 브라우저에서 `drive.google.com/thumbnail` 리다이렉트가 깨지는 경우. 목적지 `lh3.googleusercontent.com/d/<id>=w1200`
  을 바로 쓰고, 실패 시 Drive 주소 재시도, 둘 다 실패면 안내 문구. 배포본에서 표시 확인(사용자 실측).
