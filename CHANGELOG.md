...
### 2026-09-14 — CSR 목록 툴바 · 업무 Flow Diagram

### Added

- **업무 Flow Diagram** (`/csr/flow`) — 수주(SO Confirm) · 입고(Receiving) 스윔레인 도면 2종.
  원본은 SVG 를 스크립트로 그리는 단독 HTML 이라 다시 그리지 않고 `public/csr/flow/*.html` 을
  `<iframe>` 으로 띄웁니다. 도면 폭이 2500~2600px 로 고정이라 같은 출처에서 `<svg id="s">` 의
  크기를 재어 「화면 맞춤」 배율을 계산하고, ± · 100% · 새 창을 함께 둡니다.
  메뉴는 CSR(개선요청) 하부 「업무 Flow Diagram / Diagram Alur Kerja」
- **심각도(중요도) 필터** — 목록 툴바 드롭다운. 저장값 `S1 Blocker` 는 개선요청서 표기와 같게
  `S1 (Blocker)` 로 보여 줍니다. 스토어의 `filters.priority` 는 이미 있던 것을 그대로 씁니다

### Removed

- **공지 화면 삭제** (`/csr/notices`) — 화면 · 메뉴 · 헤더 종 아이콘 · 미확인 점 헬퍼(`notices.js`)까지.
  옛 주소는 이슈번호 `notices` 로 잡혀 「없는 이슈」가 뜨므로 목록으로 redirect 합니다.
  `csr_notices` 표와 목록 머리줄의 고정 공지 한 줄은 그대로 두되 **읽기 전용**입니다 —
  작성 · 편집 · 삭제 화면이 사라져 앞으로는 SQL 로만 고칩니다

### Changed

- 목록 툴바 색 구분 — 프리셋(파랑) · 필터 3종(초록) · 이동(회색). 전부 같은 회색 테두리라
  역할이 구분되지 않았습니다. 글자는 Bootstrap sm 기본 14px 에서 2px 줄인 12px
- 목록 열 순서 — 담당자 ↔ 현업검증 자리 교환(IT수용여부 · IT상태 · 현업검증 · 담당자)
...
- 대메뉴 선택지에 Finance 추가
- 헤더 메뉴 재배치(2026-09-14) — 「CSR (개선요청)」 대분류를 첫 번째로(Dashboard · 목록 · 공지 · 상태 기준 · 관리), 참고·MOCK-UP 은 Settings 하위메뉴로.
  드롭다운 활성은 가장 긴 경로 하나만
- 언어 토글(국기)을 헤더로 — /csr 이하 전 페이지 공통, 대시보드·목록의 개별 버튼 제거
- **한국어/인니어 분리 마무리(2026-09-14)** — 상태 기준(작업지시서) 페이지 본문·표를 토글 언어 한쪽만(인니어 번역 추가),
  메뉴 라벨(CSR 대분류·하위 5종·참고 2종)에 `labelId` 와 `navLabel()`, 페이지 부제 4종(대시보드·목록·공지·관리),
  로그인 화면·비밀번호 변경 창, 목록 「접근 권한 없음」 안내, 공지 제목·본문(`pickLinesLang` — "KO / ID" 병기 줄만 분리)
- ID 모드 실측: CSR 5개 화면에 남은 한글 0건
- **중복 점검 · 3원 정합성 검증(2026-09-14)** — 활성 70건 전수 대조: 행 중복 0(이슈번호 · 첨부 · 검증 이력 · 회신).
  내용 중복 3건 정리(`db/027`): 11 ↔ 48 역할 분담 · 16 ↔ 62 · 66 배치 · 63 범위를 대메뉴 · 메뉴 순서로 축소.
  삭제 · 아카이브 · 병합은 하지 않음(노션 원장 운영 규칙).
  사이트 ↔ 노션 ↔ v2 문서 대조 — 제목 68건 · 중요도 30건 전건 일치, 차이 33건(사이트 앞선 것 27 · 노션 앞선 것 4 · 판단 필요 2).
  보고서 `docs/csr/정합성검증_3원_20260914.md`
- **최종 접속일(2026-09-14)** — 관리 화면 사용자 표에 「최종 접속」 열 추가. `db/029`: `csr_user_roles.last_seen_at` +
  자기 행만 찍는 SECURITY DEFINER 함수 `csr_touch_last_seen()`(csr_user_roles 쓰기 정책은 admin 전용이라 정책을 열지 않고 함수로).
  앱은 세션 확인 때 한 번 호출(실패해도 화면 영향 없음) — 로그인 시각이 아니라 마지막 사용 시각입니다. 오늘·어제는 말로 표시
- **최종 접속이 찍히지 않던 문제(2026-09-15)** — 「최종 접속」이 전원 빈칸. 원인은 두 겹이었습니다.
  (1) `touchLastSeen()` 이 `.rpc()` 를 try/catch 로 감쌌는데 PostgREST 클라이언트는 실패를 throw 하지 않고
  `{ data, error }` 로 돌려줍니다 — catch 가 한 번도 걸리지 않아 실패가 아무 흔적도 남기지 않았습니다.
  오류 코드·메시지를 콘솔에 펼쳐 찍도록 고쳤습니다(화면은 종전대로 영향 없음).
  (2) 드러난 실제 원인은 `PGRST202` — 함수·권한은 정상인데 Data API 의 PostgREST 스키마 캐시에 안 올라온 상태였습니다.
  **Neon Data API 는 `NOTIFY pgrst, 'reload schema'` 를 듣지 않습니다** — 콘솔에서 Data API 를 껐다 켜야 합니다.
  이 저장소의 첫 RPC 라 이번에 드러났습니다. 경과는 `db/030`, 같은 파일의 anonymous GRANT 는 015 선례에 맞춘 것(원인 아님)
- 언어 토글의 **태극기를 다시 그림(2026-09-15)** — 종전 인라인 SVG 는 태극 S 곡선의 arc sweep 이 뒤집히고
  4괘가 가로로 깔려 실제 배치와 달랐습니다. Wikimedia 공개 도메인 원본(Flag_of_South_Korea.svg) 좌표를
  viewBox 째 옮겼습니다 — 이모지를 쓰지 않는 이유(Windows 글리프 없음)는 그대로입니다
- v2 문서(hwpx)에 범위 주석 1줄 추가 — 「69 · 70 은 사이트 검증 발생분으로 문서 미수록 · 이후 신규는 71번부터」.
  11:57 판에서 없어진 두 절(이슈번호 변경 기록 · 오픈 前 조치)은 의도한 삭제로 확인되어 복구하지 않음
- 정합성 조치(2026-09-14) — 노션에 **69 · 70 신규 등록**(61 과 같은 6단 양식), 담당자 15건 · 63 제목 · 관련이슈 3건 노션 반영,
  노션이 앞선 값 4건(62 · 63 중메뉴, 64 · 65 관련이슈) 사이트 반영, 61~70 최종검증일 공란, 69 · 70 제목 번호 접두.
  `db/028_sync_20260914.sql` — 02 오입력 되돌림 · 13 테스트 이력 삭제. **적용 완료(2026-09-14)**
- **3원 정합성 최종 확인** — 활성 70건 × 11개 속성 + 제목 불일치 **0건**(사이트 ↔ 노션), 문서(v2) 39~68 전건 일치.
  노션 운영 메모 `asm_csr_notion_db.md` 의 채번 규칙을 연번(다음 71)으로 갱신하고 외부 변경 기록 추가
- `db/026_issue_no_sequential.sql` — 이슈 번호 한 체계로: CSR-202609-001 → 69, 002 → 70, 참조 문구 치환, 채번 함수를 연번(최대값+1)으로
...
- 담당자 이름 정리 `db/022` — 서종환 → 'SEO', Tari → 'Lestari'(계정 표시 이름 · 이슈 담당자 모두). Firman 은 사용자 등록으로 해결, 45번 「Lestari · Firman」→ Firman(`db/023`)
